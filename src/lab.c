...
static u8 stc_hmn_controller;  // making this static so importing recording
                               // doesnt overwrite
static u8 stc_cpu_controller;  // making this static so importing recording
                               // doesnt overwrite
static u8 stc_null_controller; // making this static so importing recording
                               // doesnt overwrite

// Aitch: not really a better way to do this that I can think of.
// Feel free to change if you find a way to implement playback takeover without
// a global.
...
static void rebound_tech_chances(EventOption tech_menu[4], int menu_lookup[],
                                 int slot_idx_changed) {
  u16 *chances[4];
  int enabled_slots = 0;
...
void Lab_ChangeTechInPlaceChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, tech_option_menu_lookup, 0);
}
void Lab_ChangeTechAwayChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, tech_option_menu_lookup, 1);
}
void Lab_ChangeTechTowardChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, tech_option_menu_lookup, 2);
}
void Lab_ChangeMissTechChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, tech_option_menu_lookup, 3);
}

void Lab_ChangeStandChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, getup_option_menu_lookup, 0);
}
void Lab_ChangeRollAwayChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, getup_option_menu_lookup, 1);
}
void Lab_ChangeRollTowardChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, getup_option_menu_lookup, 2);
}
void Lab_ChangeGetupAttackChance(GOBJ *menu_gobj, int _new_val) {
  rebound_tech_chances(LabOptions_Tech, getup_option_menu_lookup, 3);
}

static int enabled_slot_chances(EventOption slot_menu[REC_SLOTS],
...
static void rebound_slot_chances(EventOption slot_menu[REC_SLOTS],
                                 int slot_idx_changed) {
  u16 *chances[REC_SLOTS];
...
GOBJ *InfoDisplay_Init(int ply) {
  ...
  // Create text object
  int canvas_index = Text_CreateCanvas(2, idGOBJ, 14, 15, 0, GXLINK_INFDISPTEXT,
                                       GXPRI_INFDISPTEXT, 19);
  ...
  // if ((OSGetConsoleType() == OS_CONSOLE_DEVHW3) || (stc_HSD_VI->is_prog ==
  // 1)) // 480p / dolphin uses medium by default
...
void InfoDisplay_Update(GOBJ *menu_gobj, EventOption menu[], GOBJ *fighter,
                        GOBJ *below) {
  ...
  if (Pause_CheckStatus(1) != 2) {
    ...
    if (rowsEnabled != 0) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        case (INFDISP_POS): {
          Text_SetText(text, i, "Pos: (%+.3f , %+.3f)",
                       fighter_data->phys.pos.X, fighter_data->phys.pos.Y);
          break;
        }
        case (INFDISP_STATE): {
          if (fighter_data->action_id != -1) {
            FtAction *action =
                Fighter_GetFtAction(fighter_data, fighter_data->action_id);
            ...
            // extract from the opponent fighter actions if there is no symbol
            // in the fighter action (e.g. getting Yoshi's Neutral-B, Mewtwo's
            // Side-B, Bowser's Side-B, etc.)
            if (symbol == NULL) {
              // loop through all humans
              for (int i = 0; i < 6; i++) {
                if (i == fighter_data->ply) {
                  continue;
                }
                ...
                action = Fighter_GetFtAction(other_fighter_data,
                                             fighter_data->action_id);
        ...
        case (INFDISP_SELFVEL): {
          Text_SetText(text, i, "SelfVel: (%+.3f , %+.3f)",
                       fighter_data->phys.self_vel.X,
                       fighter_data->phys.self_vel.Y);
          break;
        }
        case (INFDISP_KBVEL): {
          Text_SetText(text, i, "KBVel: (%+.3f , %+.3f)",
                       fighter_data->phys.kb_vel.X,
                       fighter_data->phys.kb_vel.Y);
          break;
        }
        case (INFDISP_TOTALVEL): {
          Text_SetText(
              text, i, "TotalVel: (%+.3f , %+.3f)",
        ...
        case (INFDISP_ENGLSTICK): {
          Text_SetText(text, i, "LStick:      (%+.4f , %+.4f)",
                       fighter_data->input.lstick.X,
                       fighter_data->input.lstick.Y);
          break;
        }
        case (INFDISP_SYSLSTICK): {
          HSD_Pad *pad = PadGet(fighter_data->ply, PADGET_MASTER);
          Text_SetText(text, i, "LStick Sys: (%+.4f , %+.4f)", pad->fstickX,
                       pad->fstickY);
          break;
        }
        case (INFDISP_ENGCSTICK): {
          Text_SetText(text, i, "CStick:     (%+.4f , %+.4f)",
                       fighter_data->input.cstick.X,
                       fighter_data->input.cstick.Y);
          break;
        }
        case (INFDISP_SYSCSTICK): {
          HSD_Pad *pad = PadGet(fighter_data->ply, PADGET_MASTER);
          Text_SetText(text, i, "CStick Sys: (%+.4f , %+.4f)", pad->fsubstickX,
                       pad->fsubstickY);
          break;
        }
        case (INFDISP_ENGTRIGGER): {
          Text_SetText(text, i, "Trigger:     (%+.3f)",
                       fighter_data->input.trigger);
          break;
        }
        case (INFDISP_SYSTRIGGER): {
          HSD_Pad *pad = PadGet(fighter_data->ply, PADGET_MASTER);
          Text_SetText(text, i, "Trigger Sys: (%+.3f , %+.3f)",
                       pad->ftriggerLeft, pad->ftriggerRight);
          break;
        }
        case (INFDISP_LEDGECOOLDOWN): {
          Text_SetText(text, i, "Ledgegrab Timer: %d",
                       fighter_data->ledge_cooldown);
          break;
        }
        case (INFDISP_INTANGREMAIN): {
          int intang = fighter_data->hurt.intang_frames.respawn;
          if (fighter_data->hurt.intang_frames.ledge >
              fighter_data->hurt.intang_frames.respawn)
        ...
        case (INFDISP_HITSTOP): {
          Text_SetText(text, i, "Hitlag: %.0f",
                       fighter_data->dmg.hitlag_frames);
          break;
        }
        ...
        case (INFDISP_SHIELDHEALTH): {
          Text_SetText(text, i, "Shield Health: %.3f",
                       fighter_data->shield.health);
          break;
        }
        ...
        case (INFDISP_ECBLOCK): {
          Text_SetText(text, i, "ECB Lock: %d",
                       fighter_data->coll_data.u.ecb_bot_lock_frames);
          break;
        }
        case (INFDISP_ECBBOT): {
          Text_SetText(text, i, "ECB Bottom: %.3f",
                       fighter_data->coll_data.ecbCurr_bot.Y);
          break;
        }
        case (INFDISP_JUMPS): {
        ...
        case (INFDISP_WALLJUMPS): {
          Text_SetText(text, i, "Walljumps: %d",
                       fighter_data->jump.walljumps_used);
          break;
        }
        case (INFDISP_JAB): {
        ...
        case (INFDISP_LINE): {
          ...
          Text_SetText(text, i, "Lines: G:%d, C:%d, L:%d, R:%d,", ground, ceil,
                       left, right);
          break;
        }
        case (INFDISP_BLASTLR): {
          Stage *stage = STAGE;
          Text_SetText(text, i, "Blastzone L/R: (%+.3f,%+.3f)",
                       stage->blastzoneLeft, stage->blastzoneRight);
          break;
        }
        case (INFDISP_BLASTUD): {
          Stage *stage = STAGE;
          Text_SetText(text, i, "Blastzone U/D: (%.2f,%.2f)",
                       stage->blastzoneTop, stage->blastzoneBottom);
          break;
        }
        }
      }
      ...
      if (below != NULL) {
        ...
        float y_above =
            belowData->menuModel->trans.Y +
            belowData->botLeftEdge->trans.Y * belowData->menuModel->scale.X;
      ...
      idData->text->trans.X =
          idData->menuModel->trans.X + scale * INFDISPTEXT_X;
      idData->text->trans.Y =
          -idData->menuModel->trans.Y + scale * INFDISPTEXT_Y;
...
static int check_IASA(FighterData *data) {
  // For some reason, the iasa flag isn't reset when the state changes, only
  // when a new action that has iasa starts. So we need to do some bs tracking
  // to figure out if the character is in iasa.
...
static int CheckOverlay(GOBJ *character, OverlayGroup overlay) {
  ...
  case (OVERLAY_ACTIONABLE): {
    ...
    if (state == ASID_LANDING &&
        data->state.frame >= data->attr.normal_landing_lag)
      return true;

    if (check_IASA(data))
      return true;

    if (state == ASID_CLIFFWAIT && data->TM.state_frame > 1)
      return true;

    if (ASID_WAIT <= state && state <= ASID_WALKFAST)
      return true;
    if (ASID_GUARD <= state && state <= ASID_GUARDREFLECT)
      return true;
    if (ASID_JUMPF <= state && state <= ASID_FALLAERIALB)
      return true;

    return state == ASID_TURN || state == ASID_DASH || state == ASID_RUN ||
           state == ASID_PASS || state == ASID_SQUATWAIT ||
  ...
  case (OVERLAY_CAN_FASTFALL): {
    ...
    // peach, yoshi, ness, mewtwo cannot fastfall in dj animation for whatever
    // reason.
    if ((kind == FTKIND_PEACH || kind == FTKIND_YOSHI || kind == FTKIND_NESS ||
         kind == FTKIND_MEWTWO) &&
  ...
  case (OVERLAY_WALK):
    return state == ASID_WALKSLOW || state == ASID_WALKMIDDLE ||
           state == ASID_WALKFAST;

  case (OVERLAY_DASH):
    return state == ASID_DASH;
  case (OVERLAY_RUN):
    return state == ASID_RUN;
...
int CPUAction_CheckASID(GOBJ *cpu, int asid_kind) {
  ...
  case (ASID_ANY):
    return true;
  case (ASID_DAMAGEAIR):
    return in_air && in_hitstun_anim(cpu) && hitstun_ended(cpu);
  ...
  ACTIONABLEAIR: {
    ...
    return cpu_state == ASID_JUMPF || cpu_state == ASID_JUMPB ||
           cpu_state == ASID_JUMPAERIALF || cpu_state == ASID_JUMPAERIALB ||
           cpu_state == ASID_FALL || cpu_state == ASID_FALLAERIALF ||
           cpu_state == ASID_FALLAERIALB || cpu_state == ASID_DAMAGEFALL ||
           cpu_state == ASID_DAMAGEFLYROLL || cpu_state == ASID_DAMAGEFLYTOP;
  }
  ...
  ACTIONABLEGROUND: {
    ...
    if (cpu_state == ASID_LANDING &&
        cpu_data->state.frame >= cpu_data->attr.normal_landing_lag)
      return true;

    return cpu_state == ASID_WAIT || cpu_state == ASID_WALKSLOW ||
           cpu_state == ASID_WALKMIDDLE || cpu_state == ASID_WALKFAST ||
           cpu_state == ASID_RUN || cpu_state == ASID_SQUATWAIT ||
           cpu_state == ASID_OTTOTTOWAIT || cpu_state == ASID_GUARD;
  }

  // check normal action states
  default:
    return cpu_state == asid_kind;
  }
}
...
int CPU_IsThrown(GOBJ *cpu, GOBJ *hmn) {
  ...
  if (hmn_kind == 3 &&
      (cpu_state == ASID_THROWNF || cpu_state == ASID_THROWNFF))
    return 0;

  if (ASID_THROWNF <= cpu_state && cpu_state <= ASID_THROWNLW)
    return 1;
  if (ASID_THROWNFF <= cpu_state && cpu_state <= ASID_THROWNFLW)
    return 1;
  if (cpu_state == ASID_CAPTURECAPTAIN)
    return 1;
  if (cpu_state == ASID_THROWNKOOPAF)
    return 1;
  if (cpu_state == ASID_THROWNKOOPAB)
    return 1;
  if (cpu_state == ASID_THROWNKOOPAAIRF)
    return 1;
  if (cpu_state == ASID_THROWNKOOPAAIRB)
    return 1;
...
int CPU_IsGrabbed(GOBJ *cpu, GOBJ *hmn) {
  ...
  if (cpu_state == ASID_CAPTUREWAITHI)
    return 1;
  if (cpu_state == ASID_CAPTUREWAITLW)
    return 1;
  if (cpu_state == ASID_CAPTUREWAITKOOPA)
    return 1;
  if (cpu_state == ASID_CAPTUREWAITKOOPAAIR)
    return 1;
  if (cpu_state == ASID_CAPTUREWAITKIRBY)
    return 1;
  if (cpu_state == ASID_DAMAGEICE)
    return 1;
  if (cpu_state == ASID_CAPTUREMASTERHAND)
    return 1;
  if (cpu_state == ASID_YOSHIEGG)
    return 1;
  if (cpu_state == ASID_CAPTUREKIRBYYOSHI)
    return 1;
  if (cpu_state == ASID_KIRBYYOSHIEGG)
    return 1;
  if (cpu_state == ASID_CAPTURELEADEAD)
    return 1;
  if (cpu_state == ASID_CAPTURELIKELIKE)
    return 1;
  if (cpu_state == ASID_CAPTUREWAITCRAZYHAND)
    return 1;
  if (ASID_SHOULDEREDWAIT <= cpu_state && cpu_state <= ASID_SHOULDEREDTURN)
    return 1;
...
void CPUThink(GOBJ *event, GOBJ *hmn, GOBJ *cpu) {
  ...
  if ((cpu_state == ASID_GUARDSETOFF) ||
      ((cpu_data->kind == 0xE) && (cpu_state == 344))) {
    Fighter_ZeroCPUInputs(cpu_data);
    ...
    // Keep holding shield during hitstop/hitlag. Prevents nana from dropping
    // shield.
  ...
  if ((cpu_state == ASID_DOWNBOUNDD) || (cpu_state == ASID_DOWNBOUNDU) ||
      (cpu_state == ASID_DOWNWAITU) || (cpu_state == ASID_DOWNWAITD) ||
      (cpu_state == ASID_PASSIVE) || (cpu_state == ASID_PASSIVESTANDB) ||
  ...
  CPULOGIC_START: {
    ...
    case (CPUBEHAVE_SHIELD): {
      ...
      // if you do not check action state before inputting a shield angle, it
      // can cause slight bugs with OOS options, like trying to input shield
      // angle while doing a grounded up-b OOS.
      if (shield_ang != CPUSHIELDANG_NONE &&
          (CPUAction_CheckASID(cpu, ASID_ACTIONABLEGROUND) ||
           CPUAction_CheckASID(cpu, ASID_GUARD))) {
        s8 stickX = 0;
        s8 stickY = 0;
        if (shield_ang == CPUSHIELDANG_TOWARD ||
            shield_ang == CPUSHIELDANG_AWAY) {
  ...
  CPULOGIC_SDI: {
    ...
    if (eventData->cpu_lasthit != cpu_data->dmg.atk_instance_hurtby) {
      ...
      if ((LabOptions_CPU[OPTCPU_SDINUM].option_val > 0) &&
    ...
    if (eventData->cpu_sdinum < LabOptions_CPU[OPTCPU_SDINUM].option_val) {
      ...
      case SDIDIR_RANDOM: {
        ...
        if (cpu_data->phys.air_state == 0) {
          ...
          if (eventData->cpu_sdidir == 0)
            angle =
                lastSDIWasCardinal ? M_1DEGREE * 17 : M_1DEGREE * 0; // right
          else
            angle =
                lastSDIWasCardinal ? M_1DEGREE * 197 : M_1DEGREE * 180; // left
        }
      ...
      case SDIDIR_AWAY: {
        ...
        angle =
            lastSDIWasCardinal ? (angle + 17) * M_1DEGREE : angle * M_1DEGREE;
        magnitude = 1;
  ...
  case (CPUSTATE_TDI):
  CPULOGIC_TDI: {
    // if no more hitlag and not being thrown, enter tech state. this might
    // never be hit, just being safe
    ...
    if ((cpu_data->state_id >= ASID_GUARDON) &&
        (cpu_data->state_id <= ASID_GUARDREFLECT))
      break;
    ...
    if (is_thrown) {
      ...
      if (attacker_data->state_id != ASID_THROWB) {
        ...
        // Bthrow is special - we can't use attacker direction cuz it's
        // different between throws, e.x. peach doesn't turn around, but fox
        // does. So we need to use cpu direction. We assume that bthrows will
        // always be a "reverse" throw.
  ...
  CPULOGIC_NONE: {
    ...
    if (ASID_DAMAGEFLYHI <= state && state <= ASID_DAMAGEFLYROLL ||
  ...
  CPULOGIC_GETUP: {
    ...
    if ((cpu_data->state_id == ASID_DOWNWAITD) ||
    ...
    else if ((cpu_data->state_id >= ASID_DOWNBOUNDU) &&
             (cpu_data->state_id <= ASID_DOWNSPOTD)) {
      break;
    }
  ...
  CPULOGIC_COUNTER: {
    ...
    if (eventData->cpu_isactionable == 0) {
      ...
      eventData->cpu_isactionable =
          1; // set actionable flag to begin running code
      eventData->cpu_groundstate =
          cpu_data->phys.air_state; // remember initial ground state
    }

    // if started in the air, didnt finish action, but now grounded, perform
    // ground action
    ...
    if (eventData->cpu_hitkind == HITKIND_DAMAGE) {
      if ((eventData->cpu_hitnum < LabOptions_CPU[OPTCPU_CTRHITS].option_val) ||
          (eventData->cpu_sincehit <
           LabOptions_CPU[OPTCPU_CTRFRAMES].option_val)) {
        break;
      }
      ...
      if (cpu_data->phys.air_state == 0 ||
          (eventData->cpu_groundstate ==
           0)) // if am grounded or started grounded
      {
        int grndCtr = LabOptions_CPU[OPTCPU_CTRGRND].option_val;
        action_id = CPUCounterActionsGround[grndCtr];
      } else if (cpu_data->phys.air_state ==
                 1) // only if in the air at the time of hitstun ending
      {
      ...
      if (eventData->cpu_hitshieldnum <
          LabOptions_CPU[OPTCPU_SHIELDHITS].option_val) {
...
int Update_CheckPause() {
  ...
  if ((Pause_CheckStatus(1) != 2) && (*stc_dblevel >= 3) &&
      (pad->down & HSD_BUTTON_START)) {
...
int Update_CheckAdvance() {
  ...
  static int stc_advance_btns[] = {HSD_TRIGGER_L, HSD_TRIGGER_Z, HSD_BUTTON_X,
                                   HSD_BUTTON_Y};
  int advance_btn =
      stc_advance_btns[LabOptions_General[OPTGEN_FRAMEBTN].option_val];

  // check if holding L
  if (!LabOptions_General[OPTGEN_FRAMEBTN].disable &&
...
void DIDraw_Update() {
  ...
  if ((LabOptions_General[OPTGEN_DI].option_val ==
       1)) //  && (Pause_CheckStatus(1) != 2)
  {
    // loop through all fighters
    ...
    while (fighter != 0) {
      ...
      if ((fighter_data->flags.hitlag == 1) &&
          (fighter_data->flags.hitstun == 1)) {
        // free old
        ...
        if (Fighter_GetSlotType(fighter_data->ply) == 0) {
          int input_kind;
          if (Pause_CheckStatus(0) ==
              1) // if frame advance enabled, use master inputs
        ...
        if ((lstickX != 0) || (lstickY != 0)) // exclude input vector 0,0
        {
          ...
          if (kb_mult >= 0.00001) {
            ...
            // get values
            float tdi_input =
                pow((-1 * kb.X * lstickY) + (lstickX * kb.Y), 2) / kb_mult;
        ...
        DIDrawCalculate *DICollData =
            calloc(sizeof(DIDrawCalculate) * hitstun_frames);
        CollData ecb;
        ECBSize ecb_size;
        ...
        for (int i = 0; i < hitstun_frames; i++) {
          ...
          // update bone positions.  If loop count < noECBUpdate-remaining
          // hitlag fraes, use current ECB bottom Y offset
          if (vertices_num < (fighter_data->coll_data.u.ecb_bot_lock_frames -
                              fighter_data->dmg.hitlag_frames)) {
          ...
          // update collision
            ...
            // check for slide off
            if (((ecb.envFlags & ECB_GROUND) == 0) &&
                ((ecb.envFlags_prev & ECB_GROUND) !=
                 0)) // not touching ground this frame, was touching last frame
            {
        ...
        didraw->vertices[ply][0].Y = fighter_data->coll_data.topN_Curr.Y +
                                     fighter_data->coll_data.ecbCurr_left.Y;
        didraw->vertices[ply][1].X =
            fighter_data->coll_data.topN_Curr.X + asdi_orig.X;
        didraw->vertices[ply][1].Y = fighter_data->coll_data.topN_Curr.Y +
                                     fighter_data->coll_data.ecbCurr_left.Y +
                                     asdi_orig.Y;
        ...
        for (int i = 0; i < vertices_num; i++) {
          ...
          didraw->vertices[ply][i + 2].Y =
              DICollData[i].pos.Y +
              ((DICollData[i].ECBLeftY + DICollData[i].ECBTopY) / 2);
    ...
    if (fighter != 0) {
      ...
      if (stickX != 0 || stickY != 0) {
        ...
        // adjust rotate
          ...
          DevCam_AdjustRotate(cobj, &matchCam->devcam_rot,
                              &matchCam->devcam_pos, stickX, stickY);
        }
      }
    }
  }
...
void Lab_SelectCustomTDI(GOBJ *menu_gobj) {
  ...
  GObj_AddGXLink(tdi_gobj, GXLink_Common, GXLINK_MENUMODEL,
                 GXPRI_MENUMODEL);                 // add gx link
  ...
  Text_AddSubtext(text_curr, -460, 285,
                  "A = Save Input  X = Delete Input  B = Return");
...
void CustomTDI_Update(GOBJ *gobj) {
  ...
  for (int i = 0; i < TDI_DISPNUM; i++) {
    ...
    if (i < stc_tdi_val_num) {
      ...
      // update rotation
      lstick_prev->rot.Y =
          ((float)(stc_tdi_vals[this_hit][0][0]) * 1 / 80) * 0.75;
      lstick_prev->rot.X =
          ((float)(stc_tdi_vals[this_hit][0][1]) * 1 / 80) * 0.75 * -1;
      cstick_prev->rot.Y =
          ((float)(stc_tdi_vals[this_hit][1][0]) * 1 / 80) * 0.75;
      cstick_prev->rot.X =
          ((float)(stc_tdi_vals[this_hit][1][1]) * 1 / 80) * 0.75 * -1;
...
void Inputs_Think(GOBJ *gobj) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ltrig_joint->trans.X =
        (pad->ftriggerLeft * 0.5) + controller->ltrig_origin.X;
    ltrig_joint->trans.Z =
        (pad->ftriggerLeft * 1.5) + controller->ltrig_origin.Y;

    // move rtrigger
    JOBJ *rtrig_joint;
    JOBJ_GetChild(controller_jobj, &rtrig_joint, button_lookup[BTN_R].jobj, -1);
    rtrig_joint->trans.X =
        (pad->ftriggerRight * -0.5) + controller->rtrig_origin.X;
    rtrig_joint->trans.Z =
        (pad->ftriggerRight * 1.5) + controller->rtrig_origin.Y;
...
void Inputs_Init() {
  ...
  GObj_AddGXLink(input_gobj, GXLink_Common, INPUT_GXLINK,
                 INPUT_GXPRI); // add gx link
...
GOBJ *Record_Init() {
  ...
  COBJDesc ***dmgScnMdls =
      Archive_GetPublicAddress(*stc_ifall_archive, 0x803f94d0);
  ...
  JOBJ *seek_bound[2];
  JOBJ_GetChild(playback, &seek_bound, REC_LEFTBOUNDJOINT, REC_RIGHTBOUNDJOINT,
                -1);
  Vec3 seek_bound_pos;
  ...
  // Create text
  int canvas_index =
      Text_CreateCanvas(2, rec_gobj, 14, 15, 0, REC_GXLINK, GXPRI_RECTEXT, 19);
  Text *text = Text_CreateText(2, canvas_index);
  text->align = 1;
  text->kerning = 1;
  ...
  // Get text positions
  JOBJ *text_joint[2];
  JOBJ_GetChild(playback, &text_joint, REC_LEFTTEXTJOINT, REC_RIGHTTEXTJOINT,
                -1);
  ...
  GX_AllocImageData(&snap_image, EXP_SCREENSHOT_WIDTH, EXP_SCREENSHOT_HEIGHT, 4,
                    2006);
  ...
  int x = -50;
  int y = 410;
  DevText *dev_text = DevelopText_CreateDataTable(x, y, 0, width, height,
  HSD_MemAlloc(height * width * 2)); stc_devtext = dev_text;
...
void Record_CObjThink(GOBJ *gobj) {
  ...
  if ((rec_state->is_exist == 1) &&
      ((LabOptions_Record[OPTREC_CPUMODE].option_val != 0) ||
...
void Record_Think(GOBJ *rec_gobj) {
  ...
  int modes_allow_loop = hmn_mode != RECMODE_HMN_RECORD &&
                         cpu_mode != RECMODE_CPU_CONTROL &&
                         cpu_mode != RECMODE_CPU_RECORD;
  ...
  if (loop_mode & modes_allow_loop & past_last_input) {
    ...
    if (LabOptions_Record[OPTREC_HMNSLOT].option_val == 0)
      rec_data.hmn_rndm_slot =
          Record_GetRandomSlot(&rec_data.hmn_inputs, LabOptions_SlotChancesHMN);
    if (LabOptions_Record[OPTREC_CPUSLOT].option_val == 0)
      rec_data.cpu_rndm_slot =
          Record_GetRandomSlot(&rec_data.cpu_inputs, LabOptions_SlotChancesCPU);
  }
  ...
  if (modes_allow_loop) {
    ...
    // start restore timer on restore, and increment it if it has already
    // started
    ...
    if (rec_data.restore_timer >= AUTORESTORE_DELAY) {
      ...
      // reroll
      if (LabOptions_Record[OPTREC_HMNSLOT].option_val == 0)
        rec_data.hmn_rndm_slot = Record_GetRandomSlot(
            &rec_data.hmn_inputs, LabOptions_SlotChancesHMN);
      if (LabOptions_Record[OPTREC_CPUSLOT].option_val == 0)
        rec_data.cpu_rndm_slot = Record_GetRandomSlot(
            &rec_data.cpu_inputs, LabOptions_SlotChancesCPU);
    }
  }
}
...
void Record_Update(int ply, RecInputData *input_data, int rec_mode) {
  ...
  // get the frame the recording starts on. i actually hate this code and need
  // to change how this works
  int rec_start;
  if (input_data->start_frame ==
      -1) // case 1: recording didnt start, use current frame
  {
  ...
  if ((curr_frame) < (rec_start + REC_LENGTH)) {
    ...
    case (RECMODE_CPU_RECORD): {
      // recording has started BUT the player has jumped back behind it, move
      ...
      // memset(&input_data->inputs[curr_frame + 1], 0, (REC_LENGTH -
      // curr_frame) * sizeof(RecInputs));
    ...
    case (RECMODE_CPU_PLAYBACK): {
      ...
      if (curr_frame < rec_start ||
          (curr_frame - rec_start) > (input_data->num))
        return;
...
int Record_GetEndFrame() {
  ...
  // get what frame the longest recording ends on (savestate frame + recording
  // start frame + recording time)
  int hmn_end_frame = 0;
  int cpu_end_frame = 0;
...
void Record_MemcardLoad(int slot, int file_no) {
  ...
  if (CARDProbeEx(slot, &memSize, &sectorSize) == CARD_RESULT_READY) {
    ...
    if (CARDMountAsync(slot, stc_memcard_work->work_area, 0,
                       Memcard_RemovedCallback) == CARD_RESULT_READY) {
      // check card
      Memcard_Wait();
      stc_memcard_work->is_done = 0;
      if (CARDCheckAsync(slot, Memcard_RemovedCallback) == CARD_RESULT_READY) {
        ...
        for (int i = 0; i < CARD_MAX_FILE; i++) {
          ...
          if (CARDGetStatus(slot, i, &card_stat) == CARD_RESULT_READY) {
            // check company code
            if (strncmp(os_info->company, card_stat.company,
                        sizeof(os_info->company)) == 0) {
              // check game name
              if (strncmp(os_info->gameName, card_stat.gameName,
                ...
                if (strncmp("TMREC", card_stat.fileName, 5) == 0) {
                  ...
                  if (tmrec_num == file_no) {
                    file_found = 1;
                    memcpy(
                        &filename, card_stat.fileName,
                        sizeof(filename)); // copy filename to load after this
  ...
  if (file_found == 1) {
    ...
    memcard_save.xc = -1;
    Memcard_ReqSaveLoad(slot, filename, &memcard_save,
                        &stc_memcard_info->file_name, 0, 0, 0);
    ...
    if (memcard_status == 0) {
      ...
      // enable other options
      for (int i = 1; i < sizeof(LabOptions_Record) / sizeof(EventOption);
           i++) {
        LabOptions_Record[i].disable = 0;
      }
      ...
      ExportMenuSettings *menu_settings =
          transfer_buf + header->lookup.ofst_menusettings;
      ...
      // copy recordings
      for (int i = 0; i < REC_SLOTS; i++) {
        memcpy(rec_data.hmn_inputs[i], &loaded_recsave->hmn_inputs[i],
               sizeof(RecInputData));
        memcpy(rec_data.cpu_inputs[i], &loaded_recsave->cpu_inputs[i],
               sizeof(RecInputData));
      }
      ...
      LabOptions_Record[OPTREC_AUTORESTORE].option_val =
          menu_settings->auto_restore;
  ...
  if (Pause_CheckStatus(1) != 2) {
    if (lockout_timer > 0) {
      ...
      for (int port = 0; port < 4; port++) {
        ...
        if (pad == NULL)
          continue; // Skip if no controller in this port

        // check for savestate
        int blacklist =
            (HSD_BUTTON_DPAD_DOWN | HSD_BUTTON_DPAD_UP | HSD_TRIGGER_Z |
             HSD_TRIGGER_R | HSD_BUTTON_A | HSD_BUTTON_B | HSD_BUTTON_X |
             HSD_BUTTON_Y | HSD_BUTTON_START);
...
void ImageScale(RGB565 *out_img, RGB565 *in_img, int OutWidth, int OutHeight,
...
void Export_Init(GOBJ *menu_gobj) {
  ...
  GObj_AddGXLink(export_gobj, GXLink_Common, GXLINK_MENUMODEL,
                 GXPRI_MENUMODEL);             // add gx link
  menu_data->custom_gobj_think = Export_Think; // set callback

  // save jobj pointers
  JOBJ_GetChild(export_joint, &export_data->memcard_jobj[0], EXP_MEMCARDAJOBJ,
                -1);
  JOBJ_GetChild(export_joint, &export_data->memcard_jobj[1], EXP_MEMCARDBJOBJ,
                -1);
  JOBJ_GetChild(export_joint, &export_data->screenshot_jobj, EXP_SCREENSHOTJOBJ,
                -1);
  ...
  // copy recordings
  for (int i = 0; i < REC_SLOTS; i++) {
    memcpy(&temp_rec_save->hmn_inputs[i], rec_data.hmn_inputs[i],
           sizeof(RecInputData));
    memcpy(&temp_rec_save->cpu_inputs[i], rec_data.cpu_inputs[i],
           sizeof(RecInputData));
  }
  ...
  int compress_size =
      Export_Compress(recording_buffer, temp_rec_save, sizeof(RecordingSave));
  ...
  ImageScale(new_img, orig_img, RESIZE_WIDTH, RESIZE_HEIGHT,
             EXP_SCREENSHOT_WIDTH, EXP_SCREENSHOT_HEIGHT);
  resized_image.img_ptr = new_img; // store pointer to resized image
  export_data->screenshot_jobj->dobj->mobj->tobj->imagedesc =
      &resized_image; // replace pointer to imagedesc
  ...
  stc_transfer_buf_size = sizeof(ExportHeader) + img_size +
                          sizeof(ExportMenuSettings) + compress_size;
  ...
  header->metadata.stage_internal =
      Stage_ExternalToInternal(header->metadata.stage_external);
  ...
  header->lookup.ofst_menusettings =
      sizeof(ExportHeader) + img_size + compress_size;
  ...
  // menu settings
  ExportMenuSettings *menu_settings =
      stc_transfer_buf + header->lookup.ofst_menusettings;
  ...
  menu_settings->auto_restore =
      LabOptions_Record[OPTREC_AUTORESTORE].option_val;
  // recording data
  memcpy(stc_transfer_buf + header->lookup.ofst_recording, recording_buffer,
         compress_size); // compressed recording
...
int Export_SelCardThink(GOBJ *export_gobj) {
  ...
  for (int i = 1; i >= 0; i--) {
    ...
    if (CARDProbeEx(i, &memSize, &sectorSize) == CARD_RESULT_READY) {
      ...
      if (export_data->is_inserted[i] == 0) {
        ...
        // mount card
        stc_memcard_work->is_done = 0;
        if (CARDMountAsync(i, stc_memcard_work->work_area, 0,
                           Memcard_RemovedCallback) == CARD_RESULT_READY) {
          // check card
          Memcard_Wait();
          stc_memcard_work->is_done = 0;
          if (CARDCheckAsync(i, Memcard_RemovedCallback) == CARD_RESULT_READY) {
            ...
            if (CARDFreeBlocks(i, &byteNotUsed, &filesNotUsed) ==
                CARD_RESULT_READY) {
  ...
  if ((inputs & HSD_BUTTON_A) || (inputs & HSD_BUTTON_START)) {
    ...
    if ((export_data->is_inserted[cursor] == 1) &&
        (export_data->free_files[cursor] >= 1) &&
  ...
  if (export_data->is_inserted[cursor] == 0) {
    ...
    Text_SetText(
        text_desc, 0, "Slot %s: %d free blocks. %d blocks will be used.",
        slots_names[cursor], export_data->free_blocks[cursor], req_blocks);
  }
  return 1;
}
...
void Export_EnterNameInit(GOBJ *export_gobj) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < 10; j++) {
      Text_AddSubtext(text_keyboard, (-(9.0 / 2.0) * 60) + (j * 60),
                      -80 + (i * 60), "");
    }
  }
  ...
  Text_AddSubtext(text_filedetails, 0, 0,
                  "Stage: %s\nHMN: %s\nCPU: %s\n\nSlot %s", stage_name,
                  hmn_name, cpu_name,
  ...
  Text_AddSubtext(
      text_desc, 0, 0,
      "A: Select  B: Backspace  Y: Caps  Start: Confirm"); // add description
  Text_AddSubtext(
      text_desc, 0, 40,
      "     X: Space  L: Cursor left  R: Cursor right"); // add description
  ...
  Text_AddSubtext(text_filename, 0, 0,
                  export_data->filename_buffer); // add title
...
int Export_EnterNameThink(GOBJ *export_gobj) {
  ...
  s32 memSize, sectorSize;
  if (CARDProbeEx(export_data->slot, &memSize, &sectorSize) !=
      CARD_RESULT_READY)
    goto EXIT;
  ...
  // if A
    ...
    if (export_data->filename_cursor < 32) {
      ...
      filename_buffer[export_data->filename_cursor] =
          keyboard_letters[cursor[1]][cursor[0]];
...
void Export_ConfirmInit(GOBJ *export_gobj) {
  ...
  GObj_AddGXLink(confirm_gobj, GXLink_Common, GXLINK_MENUMODEL,
                 GXPRI_POPUPMODEL); // add gx link
  ...
  Text_AddSubtext(confirm_text, 0, -40, "Save File to Slot %s?",
                  slots_names[export_data->slot]);
...
int Export_Process(GOBJ *export_gobj) {
  ...
  case (EXSTAT_REQSAVE): {
    ...
    ExportHeader *header = stc_transfer_buf;
    char filename[32];
    sprintf(filename, tm_filename, header->metadata.month, header->metadata.day,
            header->metadata.year, header->metadata.hour,
            header->metadata.minute,
            header->metadata.second); // generate filename based on date, time,
                                      // fighters, and stage

    // save file name to metadata
    memcpy(&header->metadata.filename, export_data->filename_buffer,
           export_data->filename_cursor);
    ...
    if (CARDProbeEx(slot, &memSize, &sectorSize) == CARD_RESULT_READY) {
      ...
      if (CARDMountAsync(slot, stc_memcard_work->work_area, 0,
        ...
        Memcard_Wait();
        stc_memcard_work->is_done = 0;
        if (CARDCheckAsync(slot, Memcard_RemovedCallback) ==
            CARD_RESULT_READY) {
          Memcard_Wait();
          // get free blocks
          s32 byteNotUsed, filesNotUsed;
          if (CARDFreeBlocks(slot, &byteNotUsed, &filesNotUsed) ==
              CARD_RESULT_READY) {
              ...
              if (CARDGetStatus(slot, i, &card_stat) == CARD_RESULT_READY) {
                ...
                if (strncmp(os_info->company, card_stat.company,
                  ...
                  if (strncmp(os_info->gameName, card_stat.gameName,
                              sizeof(os_info->gameName)) == 0) {
                    // check file name
                    if (strncmp(&filename, card_stat.fileName,
                                sizeof(filename)) == 0) {
                      // delete
    ...
    memcpy(stc_memcard_info->file_desc, export_data->filename_buffer,
           export_data->filename_cursor); // copy inputted name
    memcard_save.data = stc_transfer_buf;
    memcard_save.x4 = 3;
    memcard_save.size = stc_transfer_buf_size;
    memcard_save.xc = -1;
    Memcard_ReqSaveCreate(slot, &filename, &memcard_save, stc_memcard_unk,
                          stc_memcard_info->file_name,
                          stc_lab_data->save_banner, stc_lab_data->save_icon,
                          0);
...
void Event_PostThink(GOBJ *gobj) {
  ...
  // we do this in post so that we can use the updated character data before
  // rendering.
...
void Event_Init(GOBJ *gobj) {
  ...
  memcpy(LabOptions_OverlaysHMN, LabOptions_OverlaysDefault,
         sizeof(LabOptions_OverlaysDefault));
  memcpy(LabOptions_OverlaysCPU, LabOptions_OverlaysDefault,
         sizeof(LabOptions_OverlaysDefault));

  memcpy(LabOptions_InfoDisplayHMN, LabOptions_InfoDisplayDefault,
         sizeof(LabOptions_InfoDisplayDefault));
  memcpy(LabOptions_InfoDisplayCPU, LabOptions_InfoDisplayDefault,
         sizeof(LabOptions_InfoDisplayDefault));
  LabOptions_InfoDisplayHMN[OPTINF_PRESET].onOptionChange =
      Lab_ChangeInfoPresetHMN;
  LabOptions_InfoDisplayCPU[OPTINF_PRESET].onOptionChange =
      Lab_ChangeInfoPresetCPU;

  Memcard *memcard = R13_PTR(MEMCARD);
  LabOptions_General[OPTGEN_FRAMEBTN].option_val =
      memcard->TM_LabFrameAdvanceButton;
  ...
  // update to show/hide
  InfoDisplay_Update(infodisp_gobj_hmn, LabOptions_InfoDisplayHMN,
                     Fighter_GetGObj(0), NULL);
  InfoDisplay_Update(infodisp_gobj_cpu, LabOptions_InfoDisplayCPU,
                     Fighter_GetGObj(1), infodisp_gobj_hmn);
...
void Event_Update() {
  if (Pause_CheckStatus(1) != 2) {
    float speed =
        LabOptions_GameSpeeds[LabOptions_General[OPTGEN_SPEED].option_val];
...
void Event_Think(GOBJ *event) {
  ...
  if (invisible) {
    if (is_tech_anim(cpu_data->state_id)) {
      ...
      if (char_id >= sizeof(tech_frame_distinguishable) /
                         sizeof(*tech_frame_distinguishable))
      ...
      if (frame_distinguishable != -1) {
        ...
        cpu_data->flags.invisible =
            frame_distinguishable < state_frame && state_frame < 20;
      ...
      if (move_timer == MOVE_THRESHOLD) {
        ...
        if (cpu_data->flags.dead == 0) {
          ...
          if (hmn_data->phys.air_state == 0) {
            ...
            Vec3 line_unk;
            float fromX =
                (hmn_data->phys.pos.X) + (hmn_data->facing_direction * 16);
            float toX = fromX;
            float fromY = (hmn_data->phys.pos.Y + 5);
            float toY = fromY - 10;
            isGround = GrColl_RaycastGround(&coll_pos, &line_index, &line_kind,
                                            &line_unk, -1, -1, -1, 0, fromX,
                                            fromY, toX, toY, 0);
            if (isGround == 1) {

              // do this for every subfighter (thanks for complicated code ice
              // climbers)
              int is_moved = 0;
              for (int i = 0; i < 2; i++) {
                ...
                if (this_fighter != 0) {
                  ...
                  if ((this_fighter_data->flags.sleep == 0) &&
                      (this_fighter_data->flags.dead == 0)) {
                    ...
                    // facing player
                    this_fighter_data->facing_direction =
                    ...
                    // update ECB
                    this_fighter_data->coll_data.topN_Curr =
                        this_fighter_data->phys
                            .pos; // move current ECB location to new position
                    Coll_ECBCurrToPrev(&this_fighter_data->coll_data);
                    this_fighter_data->cb.Coll(this_fighter);

                    // update camera box
                    Fighter_UpdateCameraBox(this_fighter);
                    this_fighter_data->camera_subject->boundleft_curr =
                        this_fighter_data->camera_subject->boundleft_proj;
                    this_fighter_data->camera_subject->boundright_curr =
                        this_fighter_data->camera_subject->boundright_proj;
                    ...
                    Fighter_CPUInitialize(this_fighter_data, cpu_kind,
                                          cpu_level, 0);

                    // place subfighter in the Z axis
                    if (this_fighter_data->flags.ms == 1) {
                      ftCommonData *ft_common = *stc_ftcommon;
                      this_fighter_data->phys.pos.Z =
                          ft_common->ms_zjostle_max * -1;
                    }
                  }
                }
              }
  ...
  if (!cpu_control) {
    ...
    int buttons =
        hmn_pad->held &
        (HSD_BUTTON_A | HSD_BUTTON_B | HSD_BUTTON_X | HSD_BUTTON_Y |
  ...
  for (int i = 0; i < REC_SLOTS; i++) {
    ...
    if (rec_data.hmn_inputs[i]->num != 0 &&
        LabOptions_SlotChancesHMN[slot_menu_idx].disable) {
      u16 *chances[REC_SLOTS];
      int chance_count =
          enabled_slot_chances(LabOptions_SlotChancesHMN, chances);

      LabOptions_SlotChancesHMN[slot_menu_idx].disable = 0;
      if (chance_count) {
        LabOptions_SlotChancesHMN[slot_menu_idx].option_val =
            100 / chance_count;
        chances[chance_count++] =
            &LabOptions_SlotChancesHMN[slot_menu_idx].option_val;
    ...
    if (rec_data.cpu_inputs[i]->num != 0 &&
        LabOptions_SlotChancesCPU[slot_menu_idx].disable) {
      u16 *chances[REC_SLOTS];
      int chance_count =
          enabled_slot_chances(LabOptions_SlotChancesCPU, chances);

      LabOptions_SlotChancesCPU[slot_menu_idx].disable = 0;
      if (chance_count) {
        LabOptions_SlotChancesCPU[slot_menu_idx].option_val =
            100 / chance_count;
        chances[chance_count++] =
            &LabOptions_SlotChancesCPU[slot_menu_idx].option_val;
        distribute_chances(chances, chance_count);
...
static int is_tech_anim(int state) {
  return state == ASID_PASSIVE || state == ASID_PASSIVESTANDF ||
         state == ASID_PASSIVESTANDB;
}
...
static void distribute_chances(u16 *chances[], unsigned int chance_count) {
  ...
  // Aitch: We could always start on the first chance, but when adding a new
  // slot we should distribute starting on the largest (if decreasing) or
  // smallest (if increasing) option. This fixes percents when rounding: for
  // example, three options will have chances 33, 33, and 34. When we add a new
  // event to this, if we always start on the first option, then the resulting
  // percents will be 24, 25, 25, 26. When we start on the largest/smallest
  // option then they will all be 25 as expected.
  int option = 0;
  int min = 999;
  for (int t = 0; t < chance_count; ++t) {
  ...
  // Keep adding/removing from next option until all needed change to revert to
  // 100% is applied. Algorithm is pretty garbage, but it should be quick
  // enough.
  while (delta) {
...
static void rebound_chances(u16 *chances[], unsigned int chance_count,
                            int just_changed_option) {
  if (chance_count == 0)
    return;
  ...
  int delta = 100 - sum;
  // keep adding/removing from next option chance until all needed change to
  // revert to 100% is applied
  while (delta) {
...
static float get_angle_out_of_deadzone(float angle, int lastSDIWasCardinal) {
  ...
  if (angle > M_1DEGREE * 73.5 ||
      angle < M_1DEGREE * 106.5 && lastSDIWasCardinal) {
    angle = M_1DEGREE * 73;
  } else if (angle > M_1DEGREE * 163.5 ||
             angle < M_1DEGREE * 196.5 && lastSDIWasCardinal) {
    angle = M_1DEGREE * 163;
  } else if (angle > M_1DEGREE * 253.5 ||
             angle < M_1DEGREE * 296.5 && lastSDIWasCardinal) {
    angle = M_1DEGREE * 253;
  } else if (angle > M_1DEGREE * 343.5 ||
             angle < M_1DEGREE * 16.5 && lastSDIWasCardinal) {
    angle = M_1DEGREE * 343;
  }
  return angle;
}
...
static u32 lz77_compress(u8 *uncompressed_text, u32 uncompressed_size,
                         u8 *compressed_text, u8 pointer_length_width) {
  u16 pointer_pos, temp_pointer_pos, output_pointer, pointer_length,
      temp_pointer_length;
  u32 compressed_pointer, output_size, coding_pos, output_lookahead_ref,
