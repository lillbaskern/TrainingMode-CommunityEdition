...
typedef enum PauseKind {
  ...
  PAUSEKIND_GAME, // match pause (i dont think any other scene uses this?)
                  // allows p_links
                  // 0(sys),2,16,18(matchcam),19(misccam),20(hudcam),21(coincam),22(screenflashcam),24(devtext)+
                  // to run
  PAUSEKIND_3,    // unknown what uses this, it whitelists everything
  PAUSEKIND_MATCHEND, // is used when the match ends, it allows p_links
                      // 0(sys),2,12(effect2),13(mapmisc),14(misc),15(hud),16,17,18(matchcam),19,20,21,22,24+
                      // to run
  PAUSEKIND_5,        //
  PAUSEKIND_6,        //
  PAUSEKIND_7,        //
...
struct HSD_Update {
  ...
  u32 x1c;                              // 0x1C
  u64 plink_whitelist; // 0x20, code @ 801a4eac determines which gobj plinks to
                       // allow when changing the pause state.
  u32 x28;
  u32 x2c;
  void (*onFrame)(); // 0x30
};
...
struct HSD_PollData // unofficial name, not sure what its actually called
{
  ...
  u8 x6;             //
  u8 x7;             //
  PADStatus (*pad_status)[5][4]; // 0x8, circular buffer of recent pad statuses.
                                 // array of 5
};
...
static u64 *stc_pause_plink_whitelists =
    0x803da888; // array of u64 bitfields defining which gobj p_links should run
                // for the corresponding PauseKind
...
HSD_Archive *Archive_LoadInitReturnSymbol(
    char *filename, void *ptr,
    ...); // input each symbol name pointer sequentially and terminate with 0;
void Archive_GetSections(
    HSD_Archive *archive, void *symbol_out, char *symbol_name,
    ...); // input each symbol name sequentially and terminate with 0;
...
char *
Archive_GetExtern(HSD_Archive *archive,
                  int index); // gets name of the nth symbol in the dat file
void Archive_LocateExtern(HSD_Archive *archive, char *symbols,
                          void *addr); // relocates pointers to symbols
...
void HSD_ImageDescCopyFromEFB(
    _HSD_ImageDesc *image_desc, int left, int top,
    int clear_efb); // must be called from a cobj callback!
...
void HSD_StateSetZMode(GXBool compare_enable, GXCompare func,
                       GXBool update_enable);
void HSD_StateSetNumChans(u8 nChans);
void HSD_SetupChannel(void *unk);
void HSD_StateSetColorUpdate(
    GXBool update_enable); // This function enables or disables color-buffer
                           // updates when rendering into the Embedded Frame
                           // Buffer (EFB)
void HSD_StateSetAlphaUpdate(
    GXBool update_enable); // This function enables or disables alpha-buffer
                           // updates when rendering into the Embedded Frame
                           // Buffer (EFB)
void HSD_StateSetDstAlpha(GXBool enable, u8 alpha);
void HSD_StateSetAlphaCompare(GXCompare func0, u8 reference0,
                              GXAlphaOperation operation, GXCompare func1,
                              u8 reference1);
...
void GX_AllocImageData(
    _HSD_ImageDesc *image_desc, int width, int height, int fmt,
    int size); // image data buffer is stored to the image_desc
void GXTexModeSync();
void GXPixModeSync();
